...
export default function AboutPreview() {
    ...
    return (
        <section ref={containerRef} className="py-16 md:py-32 px-6 md:px-20 bg-zinc-50/50 dark:bg-zinc-900/20 overflow-hidden relative">
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 items-center">

                {/* Text Content */}
                <div className="about-content space-y-8 order-2 lg:order-1">
                    <div className="space-y-4">
                        <h2 className="text-sm font-medium uppercase tracking-[0.2em] text-primary">{t("about.title")}</h2>
                        <h3 className="text-4xl md:text-5xl font-heading font-bold leading-tight">
                            {t("about.heading1")} <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-500">{t("about.heading2")}</span>
                        </h3>
                    </div>
                    ...
                    <Button asChild size="lg" className="rounded-full text-base px-8 h-12 group mt-4">
                        <Link href="/about">
                            {t("about.journey")} <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                        </Link>
                    </Button>
                </div>
